...
export class UserService {
  ...
  async ensureContainer(address) {
    ...
    // 2) 容器不存在：创建新容器
    let port = this.state.swtcUsers?.[address]?.port ?? this.state.nextPort ?? CONFIG.docker.basePort
  ...
  async finalizeTenant(address, name, port) {
    ...
    if (!ready) {
      throw new Error(`SWTC container ${name} did not become ready on port ${port} within ${CONFIG.docker.startupTimeoutMs}ms`)
    }
    return port
  }
  ...
  async restoreFromDocker() {
    ...
    for (const name of names) {
      ...
      if (savedTier && savedTier !== actualTier) {
        console.log(`[restore] ${address} tier mismatch: state=${savedTier}, docker=${actualTier}, updating to ${targetTier}`)
        try {
